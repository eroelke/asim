...
% function in = dme_mars_guided_analytic_in()
%#codegen
...
in.s.term.or.value(1) = -1500; % double, m, terminate at -1.5 km altitude
...
in.v.aero.cd = 1.7; % nd, from EDL challenges paper, not used
...
in.v.gnc.g.p.prop_mode = uint8(8); % nd, ADM guidance
in.v.gnc.g.p.tgt_lat = tgt_lat; % rad, for range calculations
in.v.gnc.g.p.tgt_lon = tgt_lon; % rad, for range calculations
% SEJ-E algorithm
in.v.gnc.g.p_adm.mode = uint8(1); % nd, guidance mode
in.v.gnc.g.p_adm.iter_max = uint8(5); % nd, maximum iterations
in.v.gnc.g.p_adm.A_sens_atm = 0.5; % m/s^2
in.v.gnc.g.p_adm.tgt_lat = tgt_lat; % rad
in.v.gnc.g.p_adm.tgt_lon = tgt_lon; % rad
% in.v.gnc.g.p_adm.tol = 0.5e3; % m, tolerance for corrector
in.v.gnc.g.p_adm.tol = 10e3; % m, tolerance for corrector, 10 km for now
in.v.gnc.g.p_adm.surface_g = 9.8; % m/s2, surface acceleration
in.v.gnc.g.p_adm.R = 6878.1e3;
in.v.gnc.g.p_adm.H = 7500;
in.v.gnc.g.p_adm.rho_ref = 1.225;
in.v.gnc.g.p_adm.beta1 = in.v.mp.m_ini / (in.v.aero.cd*pi*(4.5/2)^2);
in.v.gnc.g.p_adm.beta2 = in.v.mp.m_jettison / (in.v.aero.cd*pi*(2.65/2)^2);
in.v.gnc.g.p_adm.V_j_min = 500; % m/s, minimum skirt jettison velocity
in.v.gnc.g.p_adm.V_chute = 450; % m/s, nominal parachute deploy velocity
in.v.gnc.g.p_adm.alt_min = -1500; % m, minimum allowable altitude for predictor
in.v.gnc.g.p_adm.alt_max = 130e3; % m, maximum allowable altitude for predictor
in.v.gnc.g.p_adm.K_dens_min = 0.2; % nd, minimum allowable density correction factor
in.v.gnc.g.p_adm.K_dens_max = 2.0; % nd, maximum allowable density correction factor
in.v.gnc.g.p_adm.K_dens_gain = 0.05; % nd, density correction factor filter gain
in.v.gnc.g.p_adm.t_max = 2000; % s, maximum allowable time for predictor
in.v.gnc.g.p_adm.t_jettison_ini = 9999; % s, initial jettison time (big number)
in.v.gnc.g.p_adm.npole = [0;0;1]; % nd, unit vector along north pole of planet
in.v.gnc.g.p_adm.omega = in.p.omega; % rad/s, planet rotation rate about npole
in.v.gnc.g.p_adm.area_ref = [pi*(4.5/2)^2; pi*(2.65/2)^2]; % m^2, initial (1) and final (2) aerodynamic area reference
...
for ii = 1:length(alt_domain)
   in.v.gnc.g.p_adm.atm_table(ii,1) = alt_domain(ii); % m
   in.v.gnc.g.p_adm.atm_table(ii,2) = interp1(temp.table(:,1),temp.table(:,2),alt_domain(ii)); % kg/m^3
end
clear temp;
