% cv_dma_titan_in.m
%   input structure for continuous drag modulation at Titan
% 
...
% 
function in = cv_dma_titan_in()

in = default_in;    % default input structure

% atmospheric data
in.p = get_planetary_data( 6, 3 ); % Load settings for Titan, GRAM atmosphere + winds
atm_data = load('./data/atm_titan_gram_10000mc_reg_winds.mat');     % Atmospheric density profiles with dispersion applied
in.p.atm.table = atm_data.atm_mc_reg_tables.nom_table;
...
in.s.term.or.value(1) = 1270e3; % double, m, terminate at atmospheric exit
