...
%       iter_max: number of internal iterations per guidance call
...
in0.s.term.or.value(1) = sim.alt_min*1e3; % double, m, terminate at 0m altitude
...
in0.s.term.or.value(2) = sim.alt_max*1e3; % double, m, terminate at top of atmosphere
...
ha_tgt = gnc.ha_tgt;
in0.v.gnc.g.p_cvdma.mode = gnc.guid_mode;    % npc
in0.v.gnc.g.p_cvdma.iter_max = gnc.iter_max;
in0.v.gnc.g.p_cvdma.A_sens_atm = 0.25;
in0.v.gnc.g.p_cvdma.ha_tgt = ha_tgt*1e3;    %km -> m
in0.v.gnc.g.p_cvdma.ha_tol = gnc.ha_tol*1e3;    %km -> m
...
in0.v.gnc.g.p_cvdma.planet.alt_min = sim.alt_min*1e3; % m
in0.v.gnc.g.p_cvdma.planet.alt_max = sim.alt_max*1e3; % m
...
if (sim.efpa_flag)
    fpa_tol = 0.05; %deg
%     [fpa_f,~,haf_err,~] = fpa_given_haf(x0.efpa, ha_tgt, in0, fpa_tol, gnc.ha_tol);    
    fpa_bounds = [x0.efpa/2.5, x0.efpa*2.5];
    [fpa_f, ha, ~, ~, ~] = find_fpa_given_ha(fpa_bounds, ha_tgt, in0, fpa_tol, gnc.ha_tol); 
    if (isnan(fpa_f))
