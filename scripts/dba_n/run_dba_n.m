%% run_dba_n.m
%   -n bank angle switch aerocapture
%
...
%       pci:     0 or planet-centered inertial state
...
%
% Created Jan 2020, Evan Roelke
% 
function out = run_dba_n(x0,gnc,aero,sim,mc)
format longg
...
in0 = dba_n_in(max_alt,sim.planet,sim.atm_mode); % nominal dej-n input structure
...
in0.v.gnc.g.p_dba_n.cds(1:stages) = aero.cds;
in0.v.gnc.g.p_dba_n.cls(1:stages) = aero.cls;
in0.v.gnc.g.p_dba_n.masses(1:stages) = aero.m'; % kg
in0.v.gnc.g.p_dba_n.area_refs(1:length(areas)) = transpose(areas); % m^2
in0.v.gnc.g.p_dba_n.t_max = sim.t_max;
in0.v.gnc.g.p_dba_n.bank_angles = [180; 0; 0; 0; 0] .* pi/180;

% Simulation Rates
in0.s.traj.rate = sim.traj_rate;           % double, Hz, integration rate
in0.v.gnc.g.p.rate = gnc.guid_rate;         % nd, guidance call rate (Hz)
in0.v.gnc.g.p_dba_n.traj_rate = sim.traj_rate;  % guidance integration rate (Hz)
...
% dej-n properties
in0.v.gnc.g.p_dba_n.n_jett = uint8(gnc.n);
in0.v.gnc.g.p.prop_mode = uint8(0);
in0.v.gnc.g.p.dm_mode = uint8(4);   % dej-n
in0.v.gnc.g.p_dba_n.iter_max = uint8(gnc.iters); % NPC iteration limit
in0.v.gnc.g.p_dba_n.tj_ini(1:length(gnc.tj0)) = gnc.tj0;
in0.v.gnc.g.p_dba_n.tgt_ap = gnc.ha_tgt*1000; % m, target apoapse
in0.v.gnc.g.p_dba_n.tol_ap = gnc.ha_tol*1000; % m, apoapse tolerance
in0.v.gnc.g.p_dba_n.npc_mode = uint8(1);   % root finding method (1 = newton, 0=bisection)
in0.v.gnc.g.p_dba_n.t_init = gnc.t_init;
% check efpa solving
if (sim.efpa_flag)
    fpa_tol = 0.01; %deg
%     [fpa_f,~,haf_err,~] = fpa_given_haf(x0.fpa0, gnc.ha_tgt, in0, fpa_tol, gnc.ha_tol);    
    fpa_bounds = [x0.fpa0/2, x0.fpa0*2];
...
out = main1_mex(in0);
% fprintf('Running nominal case in debug mode...\n'); out = main1(in0);         % debug mode
...
out.idxend = idxend;
for i = 1:gnc.n
    tjett = (find(out.g.dba_n.stage == i,1)-1)/(in0.s.data_rate);
    if ~isempty(tjett)
        out.t_jett(i) = tjett;
...
%     dat = struct();
    ...
    in0.v.gnc.g.p_dba_n.npc_mode = gnc.npc_mode;
...
%         in_mc.v.gnc.g.p_dba_n.atm.mc_ind = rnd(i);  % atmosphere index
...
%         in_mc.p.atm.mc = mc_atm;
        ...
        %%% Run Monte Carlo %%%
%         dat = save_mc_inputs(dat, in_mc, i);
%         continue;
...
%         out.traj(i).rho = out_mc.traj.rho;
        ...
        for j = 1:n
            tjett = (find(out_mc.g.dba_n.stage == j,1)-1)/(in0.s.data_rate);
            if isempty(tjett)
                t_jett(i,j) = nan;
                idj(i,j) = nan;
            else
        ...
        ha(:,i) = out_mc.g.dba_n.r_ap./1000;
        ha_err(:,i) = out_mc.g.dba_n.dr_ap./1000;
        ncalls(:,i) = out_mc.g.dba_n.ncalls;
        tj_curr(:,i) = out_mc.g.dba_n.tj;
        K_dens(:,i) = out_mc.g.K_dens;  %estimated density variation (nd)
        rho_est(:,i) = out_mc.g.rho_est;    %estimated density (kg/m3)
%         atm_err(:,i) = out_mc.g.dba_n.atm_err;

%         rss_nom(:,i) = out_mc.g.dba_n.rss_nom.*ones(1000,1);
%         rss_K(:,i) = out_mc.g.dba_n.rss_K;
%         rss_ens(:,i) = out_mc.g.dba_n.rss_ens;
...
%     out.g.rss_nom = rss_nom;
%     out.g.rss_K = rss_K;
%     out.g.rss_ens = rss_ens;
...
% save(['C:\Users\Evan Roelke\Documents\research\mars_ac\inputs_' ... 
%     num2str(gnc.ha_tgt/1000) 'k_tgt.mat'],'dat','in0');
% 
% for i =1:1000
%     x(i) = norm(dat.r_pci(:,i)) - in0.p.r_e;
%     y(i) = norm(dat.r_pci(:,i)) - in0.p.r_p;
% end
end % dba_n_earth_auto





function dat = save_mc_inputs(dat, in, i)

dat.r_pci(:,i) = in.s.traj.r_pci_ini;
dat.v_pci(:,i) = in.s.traj.v_pci_ini;
dat.h0(i) = in.s.traj.alt;
dat.v0_pcpf(i) = in.s.traj.vel_pp_mag;
dat.gamma_pcpf(i) = in.s.traj.gamma_pp;
dat.lat0(i) = in.s.traj.lat;
dat.lon0(i) = in.s.traj.lon;
dat.az0(i) = in.s.traj.az;
dat.cl(i) = in.v.aero.cl;
dat.cd(i) = in.v.aero.cd;
dat.Aref(i) = in.v.aero.area_ref;
dat.rn(i) = in.v.aero.nose_radius;
dat.m(i) = in.v.mp.m_ini;
%}
end % run_dba_n.m
