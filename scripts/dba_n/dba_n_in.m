...
%   *Created Jan 2020, E. Roelke

function in = dba_n_in(h_atm,planet,atm_mode)
%#codegen
...
% Control
in.v.gnc.c.p.bank_mode = uint8(9);  % switch guidance

% Guidance
in.v.gnc.g.p.bank_mode = uint8(9); %  nd, switch guidance
in.v.gnc.g.p.aoa_mode = uint8(1); %  nd, constant angle-of-attack
in.v.gnc.g.p.prop_mode = uint8(0); %  nd, none
in.v.gnc.g.p.dm_mode = uint8(0);   % no dma guidance

in.v.gnc.g.p_dba_n.npc_mode = uint8(1); % newton method
in.v.gnc.g.p_dba_n.iter_max = uint8(3); % nd, max NPC internal iterations
in.v.gnc.g.p_dba_n.A_sens_atm = 0.1; % m/s^2, sensible decel
in.v.gnc.g.p_dba_n.t_init = 0;  % s, time to start guidance
in.v.gnc.g.p_dba_n.tgt_ap = 400e3; % m, target apoapsis alt
in.v.gnc.g.p_dba_n.tol_ap = 10e3; % m, apoapsis alt tolerance
in.v.gnc.g.p_dba_n.t_max = 700; %s, max predictor integration time step (unused)
in.v.gnc.g.p_dba_n.n_jett = uint8(1);   % default 1 stage to jettison
in.v.gnc.g.p_dba_n.tol = 1e-5;  % convergence tolerance
in.v.gnc.g.p_dba_n.tj_ini = [100 0 0 0 0]'; % s, initial guesses for jettison times
in.v.gnc.g.p_dba_n.traj_rate = in.s.traj.rate; % trajectory integration rate
in.v.gnc.g.p_dba_n.area_refs(1:2) = [in.v.aero.area_ref; in.v.aero.area_ref/2]; % m^2
in.v.gnc.g.p_dba_n.cds(1:2) = in.v.aero.cd; % nd, internal vehicle model cds
in.v.gnc.g.p_dba_n.cls(1:2) = in.v.aero.cl; % nd, internal vehicle model cls
in.v.gnc.g.p_dba_n.masses(1:2) = [in.v.mp.m_ini in.v.mp.m_ini/2]; % kg, first two stage masses
