...
%   Venus using manual, user-defined jettison times
%
...
in.p = get_planetary_data( 1, 3 ); % Load settings for Venus, table atm model with winds
...
in.s.traj.r_pci_ini = [(in.p.r_m + alt0); 0; 0]; % double(3), m, initial PCI position vector
...
in.v.gnc.g.p_manual.masses(1:2) = in.v.mp.m_ini; % kg
in.v.gnc.g.p_manual.n_jett = uint8(1);

% Control
in.v.gnc.c.p.rate = in.s.traj.rate;

end % manual_venus_in.m
