...
%   Titan using deceleration curve fit (DCF) guidance.
%
...
function in = dcf_sej_titan_in()
%#codegen

%% Load proper curve fit
curve = load('TitanDCF.mat');
...
in.p = get_planetary_data( 6, 3 ); % Load settings for Titan, table atm model
atm_data = load(['atm_titan_gram_1000mc.mat']);     % Atmospheric density profiles with dispersion applied
...
in.s.term.or.value(1) = 1700e3; % double, m, terminate at atmospheric exit
...
in.s.traj.rate = 25; % double, Hz, integration rate
in.s.traj.t_ini = 0; % double, s, initial time
in.s.traj.t_max = 10000; % double, s, maximum allowable time
...
in.v.gnc.g.p_dcf_sej.tj_ini = 1000; % s
