...
in0.v.gnc.g.p.atm.ecf.mode = uint8(gnc.ecf.mode);
in0.v.gnc.g.p.atm.ecf.tol = gnc.ecf.p_tol;  % tolerance on ensemble search range (% of density estimate)
in0.v.gnc.g.p.atm.ecf.pert = gnc.ecf.pert; %perturbed density profiles
...
%         out.traj(i).rho = out_mc.traj.rho;
        ...
        for j = 1:n
            ...
            % jettison
            jett_ind = (find(out_mc.g.(mode).stage == j,1)-1) + 1;
