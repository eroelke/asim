...
function [dat,rv,oe,ra,dv,m,epf,C3] = UACfun(r, Ball, rn, delta, cd, fpa, velinit)
...
in0.s.traj.alt = 1000e3; % initial altitude, m
in0.s.traj.lat = 30.969 *pi/180; % initial latitude, rad
% in0.s.traj.lon = 168.7778*pi/180; % initial longitude, rad
in0.s.traj.lon = -96.448 *pi/180;
in0.s.traj.az = 180*pi/180; % initial azimuth, deg->rad
% Account for rotation velocity of planet:
% [in0.s.traj.vel_pp_mag, in0.s.traj.gamma_pp] = ...
%     get_vel_pp_mag_ini(velinit, in0.s.traj.lat, in0.s.traj.lon,...
%     in0.s.traj.az, in0.s.traj.alt, fpa, in0.p.omega, in0.p.r_e, in0.p.r_p);
% 
...
[in0.s.traj.r_pci_ini, in0.s.traj.v_pci_ini, vel_pp, in0.s.traj.gamma_pp] = ...
    get_vel_pp_mag_ini(velinit, in0.s.traj.lat, in0.s.traj.lon,...
    in0.s.traj.az, in0.s.traj.alt, fpa, in0.p.omega, in0.p.r_e, in0.p.r_p);
in0.s.traj.vel_pp_mag = norm(vel_pp);

% Sim settings
min_alt = 0;        % minimum allowable altitude (m)
max_alt = 1000e3;    % maximum allowable altitude (m)
...
epf = dat.traj.vel_ii_mag(idxend)^2/2 - ...
    in0.p.mu / r;

C3 = dat.traj.vel_ii_mag(idxend)^2 - 2*in0.p.mu / r;
