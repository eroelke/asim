...
%             nav.s.rva_error = zeros(9,1);
        ...
        case 2 % Stochastic error model
            ...
            nav.s.x_ercv = exp(-1*nav.s.dt/nav.p.ecrv.tau) * nav.s.x_ercv + nav.s.ercv;
            nav.s.rva_error = real(sqrtm(nav.p.ecrv.P_SS))*nav.s.x_ercv;
            
        case 3 % exponentially decaying random variables
            E00 = eye(3) * exp(-nav.s.t/nav.p.n_exp.tau_r);
            E22 = eye(3) * exp(-nav.s.t/nav.p.n_exp.tau_v);
            E33 = eye(3) * exp(-nav.s.t/nav.p.n_exp.tau_a);
            ...
            nav.s.rva_error = S * randn(9,1);
            
        case 4  % multivariate gaussian approach
...
%             nav.s.rva_error = zeros(9,1);
    ...
    theta = norm(nav.p.atm.omega)*(nav.s.t_ini + nav.s.t);
    ...
    [nav.s.lat_gd, nav.s.lon] = get_lat_lon(nav.s.r_pcpf, nav.p.atm.r_e, nav.p.atm.r_p);
    nav.s.alt = get_alt(nav.s.r_pcpf, nav.p.atm.r_e, nav.p.atm.r_p, nav.s.lat_gd);
