...
%   *Modified JUN 2018, E. Roelke

function [s] = guid_ac_analytic( i, s, p, init_flag )
%#codegen
...
while (s.next_step ~= uint8(5))
    switch s.next_step
        ...
        case 2 % analytic range control - allen eggers?
...
if (s.t_j_curr < s.t) || (s.V_pf_mag <= p.V_j_min)
    ...
    switch p.mode
        ...
        case 1 % non-iterative (Putnam 2015)
            ...
            % compute values
            fpa0 = acos( dot(unit(i.R_pci), unit(i.V_pf_pci)) ) - pi/2; % fpa
            vc = sqrt(p.R*p.surface_g);  %circular velocity for planet (mu/R = g*R)
            cf = 1.3179;                % coefficient for range targeting (see Putnam 2015 - exp_integral(1) - gamma_bar
            
            % determine constant flight path angle, gs2
            Fs = sqrt(1 + p.H/(p.R*(tan(fpa0))^2)*(cf*vc^2/s.V_pf_mag^2 + (vc^2/s.V_pf_mag^2 - 1)*log(1- p.beta2*sin(fpa0)/(p.H*p.rho_ref)) )  );
            s.gs2 = -asin(sin(fpa0)*(2*Fs - 1));    % gamma* = constant fpa for allen eggers
            ...
            s.flight_range = p.R * cot(s.gs2) * ...
                ...
                log(p.R - p.H*log( s.dens_est/p.rho_ref )) );
            
            % Compute range to target
            tf = 300;                   % ballpark for now: final time?
            ang = norm(p.omega)*tf;     % angle to rotate target position vector about north pole
            
            % unit vector along estimated target position at parachute deploy (or other event?)
            % lat-lon target?
            u_tgt = s.u_tgt_ini*cos(ang) + ...
                p.npole*dot(p.npole,s.u_tgt_ini)*(1-cos(ang)) - ...
                cross(s.u_tgt_ini,p.npole)*sin(ang); 
...
dens_model = p.rho_ref*exp(-alt/p.H); % kg/m^3
...
s.dens_est = s.K_dens*dens_model;
