...
% 
function [p, s] = run_ensemble_filter(y0, t0, p, s)
%#codegen
if (p.atm.Kflag && p.atm.mode == uint8(2))
    % speed up process
    if (s.atm.ind_curr-1 ~= p.atm.mc_ind)
        ...
        h_ind = find(atms(:,1) >= alt,1,'first');
        mcs = atms(h_ind,2:end)';    % all density values this altitude
        ...
        for k = 1:length(inds)
            ...
            for jj = 1:model_ind
                % density rss
                rho_curr = interp1(atm_curr(:,1), atm_curr(:,2), s.atm.atm_hist(jj,1)); %interp to get mc atm density at atm model alt
        ...
        %{
                                    ...
                                    p5=plot(atm_true(:,1)/1000,log(atm_true(:,2)),'--','Color',[0 0.5 0],'LineWidth',1.5);
                                    p6=plot(atm_true(h_ind,1)/1000,log(atm_true(h_ind,2)),'.','Color',[0 0.5 0],'MarkerSize',15);
                                    xlabel('Altitude (km)')
...
%         sig_curr = (atms(h_ind,s.atm.ind_curr) - rho_est)/atms(h_ind,s.atm.ind_curr);
        ...
        % lowest rss error (pressure)
        % lowest rss error (temp)
